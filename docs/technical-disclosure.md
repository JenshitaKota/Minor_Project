...
- Repository commit (hash to be finalized once committed), dated 2026-08-08, implements the decentralized attestor-set governance described in §4.8: the propose/approve-add and cross-role propose/approve-remove rewrite of `contracts/contracts/AnchorRegistry.sol`, including the `attestorCount` floor guard. Automated test coverage: 36 Foundry contract tests (up from 19 — 17 new tests covering propose/approve-add success and rejection paths, owner-proposed/attestor-approved and attestor-proposed/owner-approved removal, self-approval and wrong-role rejection, and the minimum-attestor-count floor, plus 2 new fuzz tests).
